...
public class NewspaperApplicationController {
     ...
     * （用户完成付款后，无需客服手动确认，而是由支付系统主动回调后端的支付结果通知接口，自动将申请状态标记为 PAID（已支付）；而原有/audit接口中的PAID状态设置，
     * 仅作为兜底方案（比如支付回调失败、用户线下转账、支付系统异常等特殊场景），由客服手动补录状态，核心流程以自动回调为主。
     * 用户在前端完成支付（调用支付系统统一下单接口）
     *   ↓
     * 用户支付成功 → 支付系统（微信/支付宝/自研支付）触发**异步支付结果回调** → 调用你后端的「支付结果通知接口」
     *   ↓
     * 后端回调接口执行校验（签名+幂等+申请状态+金额）
     *   ↓
     * 校验通过 → 自动将申请状态更新为PAID，补全支付时间
     *   ↓
     * 【可选】通过WebSocket给用户推送「付款成功提醒」，给客服推送「用户已付款」提醒
     *   ↓
     * 【可选】更新会话映射/刷新ReceiverId过期时间
     *   ↓
     * 返回支付系统**固定成功响应**（支付系统要求：必须返回指定格式，否则会重复回调）
     *   ↓
     * 校验失败 → 返回支付系统失败响应，支付系统会按规则重试回调（一般3/5/7次）
     *
     * 关键注意：支付系统的异步回调是核心，后端必须提供公网可访问的回调接口（若为内网测试，需用穿透工具如 Ngrok），
     * 且回调接口的返回值必须严格遵循支付系统的规范（比如微信支付要求返回 XML 格式的success，支付宝要求返回 JSON 格式的success）。
     *
     * 1. 客服审核通过 → 自动给用户推送付款提醒（含支付链接）→ 用户前端下单支付
     * 2. 用户支付成功 → 支付系统异步回调后端/pay/callback接口
     * 3. 回调接口自动校验+更新状态为PAID → 自动给用户/客服推送付款成功提醒
     * 4. 特殊场景（支付回调失败/用户线下转账）→ 客服在工作台通过/audit接口手动标记PAID（兜底）
     *
     *
     PAID（已支付）→ REFUND_APPLIED（用户发起退款）→ 两种分支 →
     分支1：REFUNDED（客服审核通过+退款完成）（最终态，不可变更）
     分支2：REFUND_REJECTED（客服审核驳回退款）（可重新发起退款，非最终态）
     *
     * ）
